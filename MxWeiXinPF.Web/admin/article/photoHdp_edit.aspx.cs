...
{
    public partial class photoHdp_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("indexhdp", MXEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private bool DoAdd()
        {
            ...
            model.category_id = 1;//首页幻灯片的主键Id
            ...
            if (bll.Add(model) > 0)
            {
                ...
                AddAdminLog(MXEnums.ActionEnum.Add.ToString(), "添加" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(MXEnums.ActionEnum.Edit.ToString(), "修改" + this.channel_name + "频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == MXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("indexhdp", MXEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("修改信息成功！", "photoHdp_list.aspx?channel_id=" + this.channel_id, "Success");
            }
            else //添加
            {
                ChkAdminLevel("indexhdp", MXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("添加信息成功！", "photoHdp_list.aspx?channel_id=" + this.channel_id, "Success");
            }
        }
    }
}