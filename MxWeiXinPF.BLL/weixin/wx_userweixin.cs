...
namespace MxWeiXinPF.BLL
{
	...
	{
		private readonly MxWeiXinPF.DAL.wx_userweixin dal=new MxWeiXinPF.DAL.wx_userweixin();
		...
		/// 获得数据列表
		/// </summary>
		public List<MxWeiXinPF.Model.wx_userweixin> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<MxWeiXinPF.Model.wx_userweixin> DataTableToList(DataTable dt)
		{
			List<MxWeiXinPF.Model.wx_userweixin> modelList = new List<MxWeiXinPF.Model.wx_userweixin>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				MxWeiXinPF.Model.wx_userweixin model;
